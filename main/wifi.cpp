...
class WifiConfig {
 public:
  WifiConfig(std::string_view ssid, std::string_view pass) {
    std::memcpy(&config_.sta.ssid, ssid.data(), ssid.length());
    std::memcpy(&config_.sta.password, pass.data(), pass.length());
    config_.sta.scan_method = wifi_scan_method_t::WIFI_ALL_CHANNEL_SCAN;
  }

  wifi_config_t get_config() { return config_; }
...
void Init(void) {
  ...
  auto wifi_config = WifiConfig{kWifiSsid, kWifiPass}.get_config();

  ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_STA));
  ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &wifi_config));
  ESP_ERROR_CHECK(esp_wifi_start());

  ESP_LOGI(TAG, "WifiInitStation finished.");
   ...
   * actually happened. */
  if (connected) {
    ESP_LOGI(TAG, "connected to ap SSID:%s password:%s", kWifiSsid, kWifiPass);
  } else if (connection_failed) {
    ESP_LOGI(TAG, "Failed to connect to SSID:%s, password:%s", kWifiSsid, kWifiPass);
  } else {
